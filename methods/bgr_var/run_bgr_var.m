% methods/bgr_var/run_bgr_var.m

function out = run_bgr_var(Y, cfg)
% RUN_BGR_VAR  Bayesian Ridge Regression VAR via R/BigVAR.
%
%   Estimates a Bayesian Ridge (BGR) VAR using the BigVAR R package.
%   BGR applies a Minnesota-style ridge penalty with structured shrinkage.
%   Rolling cross-validation selects the optimal lambda (OOS MSFE criterion).
%
%   Requires: R on system PATH, BigVAR R package.
%
%   Reference: Banbura, Giannone & Reichlin (2010), J. Applied Econometrics.
%              Nicholson, Matteson & Bien (2017), BigVAR R package.
%
%   INPUT
%     Y   : [T x K] double — stationary multivariate time series
%     cfg : struct from bgr_var_config()
%
%   OUTPUT
%     out : canonical struct (see create_canonical_output)
    ...
    % BGR needs enough obs for rolling CV split
    ...
    % ------------------------------------------------------------------ %
    %  3. Auto-compute T1, T2 if not supplied                             %
    % ------------------------------------------------------------------ %
    T1 = cfg.T1;
    T2 = cfg.T2;
    if isempty(T1) || (isnumeric(T1) && isnan(T1)), T1 = floor(T / 3);     end
    if isempty(T2) || (isnumeric(T2) && isnan(T2)), T2 = floor(2 * T / 3); end
    ...
    if isempty(r_script)
        out = i_early_failure(METHOD, cfg, t_start, ...
            'bgr_var.R not found. Check cfg.r_script_path.');
        return
    end
    ...
    in_file  = fullfile(cfg.tmp_dir, sprintf('bgr_input_%s.csv',  run_id));
    cfg_file = fullfile(cfg.tmp_dir, sprintf('bgr_config_%s.csv', run_id));
    out_file = fullfile(cfg.tmp_dir, sprintf('bgr_output_%s.csv', run_id));
    ...
    % BigVAR does not use glmnet alpha or pilot — remove artifacts
    ...
    % ------------------------------------------------------------------ %
    out = i_bgr_postvalidate(out, cfg);
end
...
function r_script = i_resolve_r_script(cfg)
    ...
    candidate = fullfile(this_dir, 'bgr_var.R');
...
function out = i_bgr_postvalidate(out, cfg)
    if ~out.success; return; end

    % BGR is a ridge estimator — dense solutions are expected.
    % Warn only if *all* coefficients are exactly zero (numerical failure).
    if ~isempty(out.coefficients) && all(out.coefficients(:) == 0)
        out.warnings{end+1} = ...
            'All coefficients zero — BGR numerical failure or extreme lambda.';
        out.status_code = 2;
    end

    % Confirm struct stored correctly in hyperparameters
    if ~isfield(out.hyperparameters, 'struct') || ...
            ~strcmp(out.hyperparameters.struct, 'BGR')
        out.warnings{end+1} = ...
            'hyperparameters.struct mismatch (expected BGR).';
        out.status_code = 2;
    end
end
