% methods/adaptive_lasso_var/run_adaptive_lasso_var.m

function out = run_adaptive_lasso_var(Y, cfg)
% RUN_ADAPTIVE_LASSO_VAR  Adaptive LASSO-VAR via R/glmnet.
%
%   Two-stage equation-by-equation estimation delegated to R.
%   Stage 1: pilot (OLS or LASSO) in R
%   Stage 2: adaptive LASSO via glmnet with penalty.factor = w
%            where w_i = 1 / (|beta_pilot_i|^nu + eps)
%
%   Requires: R on system PATH, glmnet R package,
%             core/r_bridge/ functions on MATLAB path.
%
%   Reference: Zou (2006) JASA 101(476):1418-1429.
%
%   INPUT
%     Y   : [T x K] double — stationary multivariate time series
%     cfg : struct from adaptive_lasso_var_config()
%
%   OUTPUT
%     out : canonical struct (see create_canonical_output)

    t_start = tic;
    METHOD  = 'adaptive_lasso_var';

    % ------------------------------------------------------------------ %
    %  1. Config                                                           %
    % ------------------------------------------------------------------ %
    if nargin < 2 || isempty(cfg)
        cfg = adaptive_lasso_var_config();
    end
    ...
    if isempty(r_script)
        out = i_early_failure(METHOD, cfg, t_start, ...
            'adaptive_lasso_var.R not found. Check cfg.r_script_path.');
        return
    end
    ...
    in_file  = fullfile(cfg.tmp_dir, sprintf('alasso_input_%s.csv',  run_id));
    out_file = fullfile(cfg.tmp_dir, sprintf('alasso_output_%s.csv', run_id));
    cfg_file = fullfile(cfg.tmp_dir, sprintf('alasso_config_%s.csv', run_id));
    ...
    try
        ...
        cfg_r.K            = K;
        cfg_r.nu           = cfg.nu;
        cfg_r.weight_eps   = cfg.weight_eps;
    ...
    % ------------------------------------------------------------------ %
    out = i_adaptive_lasso_postvalidate(out, cfg);
end
...
function cfg = i_fill_defaults(cfg)
    defaults = adaptive_lasso_var_config();
...
function r_script = i_resolve_r_script(cfg)
    ...
    candidate = fullfile(this_dir, 'adaptive_lasso_var.R');
...
function out = i_adaptive_lasso_postvalidate(out, cfg)
    ...
    if cfg.nu <= 0
        out.warnings{end+1} = sprintf('nu=%.4f is non-positive.', cfg.nu);
        out.status_code = 2;
    end
end
